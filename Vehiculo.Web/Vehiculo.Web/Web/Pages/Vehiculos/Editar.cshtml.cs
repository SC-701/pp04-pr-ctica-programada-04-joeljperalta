...
namespace Web.Pages.Vehiculos
{
    public class EditarModel : PageModel
    {
            ...
            if (respuesta.StatusCode == HttpStatusCode.OK)
            {
                ...
                if (vehiculoResponse != null)
                {
                    marcaseleccionada = Guid.Parse(
                        marcas.Where(m => m.Text == vehiculoResponse.Marca).FirstOrDefault().Value
                    );

                    modelos = (await ObtenerModelos(marcaseleccionada))
                    ...
                    modeloseleccionado = Guid.Parse(
                        modelos.Where(m => m.Text == vehiculoResponse.Modelo).FirstOrDefault().Value
                    );
                }
            }
        ...
        public async Task<ActionResult> OnPost()
        {
            ...
            var respuesta = await cliente.PutAsJsonAsync<VehiculoRequest>(
                string.Format(endpoint, vehiculoResponse.Id),
                new VehiculoRequest
                {
